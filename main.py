...
def pearson():
    if request.method=='GET':
        return 'Send POST request to get prediction'
    if request.method == 'POST':
        ...
        return jsonify({'movies':pearson_recommender(userID).tolist()})

# Cosine recommendation
@app.route('/cosine', methods=['GET', 'POST'])
def cosine():
    if request.method=='GET':
        return 'Cosine GET Prediction'
    if request.method=='POST':
        return 'Cosine POST Prediction'
